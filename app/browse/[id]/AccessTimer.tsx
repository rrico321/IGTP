...
export function AccessTimer({
  ...
  useEffect(() => {
    if (!expiresAt) return; // No expiry = no countdown needed
    const update = () => {
      const diff = new Date(expiresAt).getTime() - Date.now();
      if (diff <= 0) {
        setTimeLeft("Expired");
  ...
  // Connected with no time limit (expiresAt is null)
  if (!expiresAt) {
    return (
      <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-green-500/10 border border-green-500/20 mb-4">
